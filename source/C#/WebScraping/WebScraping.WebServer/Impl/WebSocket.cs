...
namespace WebScraping.WebServer.Impl
{
    class WebSocket
    {
        ...
        private Func<String2, Opcode, WebSocketNode> _method;
                ...
                try
                {
                    while (true)
                    {
                        ...
                        if (opcode == (int)Opcode.MESSAGE)
                        {
                            ...
                            {
                                WebSocketNode node = this._method(buffer, Opcode.MESSAGE);
                                if (node.Broadcast)
                                {
                                    this._server.Send(node);
                                }
                                else
                                {
                        ...
                        if (opcode == (int)Opcode.BINARY)
                        {
                            ...
                            if (this._method != null)
                            {
                                WebSocketNode node = this._method(buffer, Opcode.BINARY);
                                if (node.Broadcast)
                                {
                                    this._server.Send(node);
                                }
